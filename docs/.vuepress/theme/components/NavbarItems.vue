...
import { getNavLink, resolveRepoType } from '@vuepress/theme-default/lib/client/utils/index'
...
const useNavbarConfig = (): ComputedRef<ResolvedNavbarItem[]> => {
    ...
    if (navItems.length) {
        const currentItems = navItems.find((item: NavbarItem) => item.text === currentKey)
    ...
    return computed(() => navItems.map((item) => resolveNavbarItem(router, item)))
}
...
<template>
    <nav v-if="navbarLinks.length" class="navbar-items">
        <div v-for="item in navbarLinks" :key="item.text" class="navbar-item">
            <NavbarDropdown v-if="item.children" :item="item" :class="isMobile ? 'mobile' : ''" />
