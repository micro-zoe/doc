...
路由分为四种模式：`search`、`native`、`native-scope`、`pure`，每种模式对应不同的功能，以满足尽可能多的项目需求，详情参考[虚拟路由系统](/zh/router)。
...
只有路由模式是native或native-scope我们才需要设置baseroute，详情参考[虚拟路由系统](/zh/router)。
...
设置`disable-memory-router`后，子应用将基于浏览器的路由系统进行渲染，拥有更加简洁优雅的的浏览器地址，详情参考[虚拟路由系统](/zh/router)。
